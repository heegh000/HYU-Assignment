...
int init_db(int num_buf) {
	...
	for (int i = 0; i < num_buf; i++) {
		memset(&buffer[i], 0, sizeof(buffer_t));
		clean_metadata(i);
	}
}
...
int join_table(int table_id_1, int table_id_2, char * pathname) {
	buffer_t* table1_buffer;
	buffer_t* table2_buffer;

	table1_buffer = get_buffer_pointer(table_id_1, 0);
	table2_buffer = get_buffer_pointer(table_id_2, 0);

	if(table1_buffer->header_page.root_page_num == 0) {
		table1_buffer->metadata.is_pinned--;
		table2_buffer->metadata.is_pinned--;
		
		printf("%d table have no root", table_id_1);
		return -1;
	}

	if(table2_buffer->header_page.root_page_num == 0) {
		table1_buffer->metadata.is_pinned--;
		table2_buffer->metadata.is_pinned--;

		printf("%d table have no root", table_id_2);
		return -1;
	}
	...
	table1_buffer->metadata.is_pinned--;
	table1_buffer = get_buffer_pointer(table_id_1, table1_buffer->header_page.root_page_num);

	while(!table1_buffer->node_page.is_leaf) {

		table1_buffer->metadata.is_pinned--;
		
		table1_buffer = get_buffer_pointer(table_id_1, table1_buffer->node_page.left_page_num);
	}

	table2_buffer->metadata.is_pinned--;
	table2_buffer = get_buffer_pointer(table_id_2, table2_buffer->header_page.root_page_num);

	while(!table2_buffer->node_page.is_leaf) {

		table2_buffer->metadata.is_pinned--;
		
		table2_buffer = get_buffer_pointer(table_id_2, table2_buffer->node_page.left_page_num);
	}
	...
	while(1) {
		while(1) {
			...
			if(table1_buffer->node_page.record[i].key == table2_buffer->node_page.record[j].key)
				break;

			if(table1_end && (table1_buffer->node_page.record[i].key < table2_buffer->node_page.record[j].key)) {
				join_end = 1;			
				break;
			}
			
			if(table2_end && (table1_buffer->node_page.record[i].key > table2_buffer->node_page.record[j].key)) {
				join_end = 1;
				break;
			}
			...
			if (table1_buffer->node_page.record[i].key < table2_buffer->node_page.record[j].key) {
				...
				if(!table1_end) {				
					if(i+1 == table1_buffer->node_page.num_of_key) {
						if(table1_buffer->node_page.right_sibling_num == 0) {
							table1_end = 1;
						}		
						else {
							table1_buffer->metadata.is_pinned--;
							table1_buffer = get_buffer_pointer(table_id_1, table1_buffer->node_page.right_sibling_num);
							i = 0;
						}
			...
			else if(table1_buffer->node_page.record[i].key > table2_buffer->node_page.record[j].key){
				if(!table2_end) {			
					if(j+1 == table2_buffer->node_page.num_of_key) {
						if(table2_buffer->node_page.right_sibling_num == 0) {
							table2_end = 1;
						}		
						else {
							table2_buffer->metadata.is_pinned--;
							table2_buffer = get_buffer_pointer(table_id_2, table2_buffer->node_page.right_sibling_num);
							j = 0;
						}
		...
		if(table1_buffer->node_page.record[i].key == table2_buffer->node_page.record[j].key) {
			fprintf(fp, "%ld,%s,%ld,%s\n", table1_buffer->node_page.record[i].key, table1_buffer->node_page.record[i].value, table2_buffer->node_page.record[j].key, table2_buffer->node_page.record[j].value);
			if(!table1_end) {				
				if(i+1 == table1_buffer->node_page.num_of_key) {
					if(table1_buffer->node_page.right_sibling_num == 0) {
						table1_end = 1;
...
record_t* make_record(int64_t key, char* value) {
	...
	memset(new_record, 0, sizeof(record_t));
	if (new_record == NULL) {
...
int insert_into_leaf(int table_id, page_t* target_page, record_t* record) {
	...
	for (i = target_page->num_of_key; i > insertion_point; i--) {
		memcpy(&target_page->record[i], &target_page->record[i - 1], sizeof(record_t));
	}

	memcpy(&target_page->record[insertion_point], record, sizeof(record_t));
...
int insert_into_leaf_after_splitting(int table_id, page_t* target_page, record_t* record) {
	...
	for (i = 0, j = 0; i < target_page->num_of_key; i++, j++) {
		if (j == insertion_index)
			j++;
		memcpy(&temp_records[j], &target_page->record[i], sizeof(record_t));
	}

	memcpy(&temp_records[insertion_index], record, sizeof(record_t));
	...
	for (i = 0; i < split; i++) {
		memcpy(&target_page->record[i], &temp_records[i], sizeof(record_t));
		target_page->num_of_key++;
	}

	for (i = split, j = 0; i < LEAF_ORDER; i++, j++) {
		memcpy(&new_page->record[j], &temp_records[i], sizeof(record_t));
		new_page->num_of_key++;
	}
...
int insert_into_internal(int table_id, page_t* target_page, int insertion_index, int64_t key, pagenum_t right_child_page_num) {
	...
	for (i = target_page->num_of_key; i > insertion_index; i--) {
		memcpy(&target_page->internal[i], &target_page->internal[i - 1], sizeof(internal_t));
	}
...
int insert_into_internal_after_splitting(int table_id, page_t* target_page, int insertion_index, int64_t key, pagenum_t right_child_page_num) {
	...
	for (i = 0, j = 0; i < target_page->num_of_key; i++, j++) {
		if (insertion_index == j)
			j++;
		memcpy(&temp_internals[j], &target_page->internal[i], sizeof(internal_t));
	}
	...
	for (i = 0; i < split - 1; i++) {
		memcpy(&target_page->internal[i], &temp_internals[i], sizeof(internal_t));
		target_page->num_of_key++;
	}
	...
	for (i = split, j = 0; i < INTERNAL_ORDER; i++, j++) {
		memcpy(&new_page->internal[j], &temp_internals[i], sizeof(internal_t));
		new_page->num_of_key++;
	}
...
int start_new_tree(int table_id, record_t* record) {
	...
	memcpy(&root_page->record[0], record, sizeof(record_t)); 
...
int remove_entry_from_page(int table_id, page_t* target_page, int64_t key) {
	...
	if (target_page->is_leaf) {
		...
		for (++i; i < target_page->num_of_key; i++) {
			memcpy(&target_page->record[i - 1], &target_page->record[i], sizeof(record_t));
		}
	}
	else
	{
		...
		for (++i; i < target_page->num_of_key; i++) {
			memcpy(&target_page->internal[i - 1], &target_page->internal[i], sizeof(internal_t));
		}
	}
...
int coalesce_page(int table_id, page_t* target_page, page_t* neighbor_page, pagenum_t neighbor_index, int64_t k_prime) {
	...
	if (target_page->is_leaf) {
		...
		for (int i = 0, j = neighbor_page->num_of_key; i < target_num_of_key_before; i++, j++) {
			memcpy(&neighbor_page->record[j], &target_page->record[i], sizeof(record_t));
	...
	// neighbor_page로 k_prime을 옮긴후 target_page의 key와 page_num들을 옮김
		...
		for (int i = 0, j = neighbor_page->num_of_key; i < target_num_of_key_before; i++, j++) {
			memcpy(&neighbor_page->internal[j], &target_page->internal[i], sizeof(internal_t));
			neighbor_page->num_of_key++;
			target_page->num_of_key--;
		}
...
int redistribute_page(int table_id, page_t* target_page, page_t* neighbor_page, int neighbor_index, int k_prime_index, int64_t k_prime) {
	...
	if (neighbor_index == -2) {
		...
		if (target_page->is_leaf) {
			...
			memcpy(&target_page->record[target_page->num_of_key], &neighbor_page->record[0], sizeof(record_t));

			parent_page->internal[k_prime_index].key = neighbor_page->record[1].key;

			for (i = 0; i < neighbor_page->num_of_key - 1; i++) {
				memcpy(&neighbor_page->record[i], &neighbor_page->record[i + 1], sizeof(record_t));
			}
		}
		//target_page가 internal page인 경우
		else {
			...
			for (i = 0; i < neighbor_page->num_of_key - 1; i++) {
				memcpy(&neighbor_page->internal[i], &neighbor_page->internal[i + 1], sizeof(internal_t));
			}
	...
	//target_page가 가장 왼쪽에 있는 페이지가 아닌 경우
		...
		if (target_page->is_leaf) {
			...
			memcpy(&target_page->record[0], &neighbor_page->record[neighbor_page->num_of_key - 1], sizeof(record_t));
