...
fetchint(uint addr, int *ip)
{
  ...
  if(addr >= curproc->mainth->sz || addr+4 > curproc->mainth->sz)
    return -1;
  *ip = *(int*)(addr);
  return 0;
}
...
fetchstr(uint addr, char **pp)
{
  ...
  if(addr >= curproc->mainth->sz)
    return -1;
  *pp = (char*)addr;
  ep = (char*)curproc->mainth->sz;
  for(s = *pp; s < ep; s++){
    if(*s == 0)
      return s - *pp;
  }
...
argptr(int n, char **pp, int size)
{
  ...
  if(argint(n, &i) < 0)
    return -1;
  if(size < 0 || (uint)i >= curproc->mainth->sz || (uint)i+size > curproc->mainth->sz)
    return -1;
  *pp = (char*)i;
  return 0;
}
