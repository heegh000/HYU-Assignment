...
def shade (ray, light_list, shape_list, idx, shade_list) :
    ...
    if target_shape.shape == 'Sphere':
        ...
        normal_vec = np.array([-1, 0, 0]).astype(np.float)
        ...
        if  abs(target_shape.min_p[1]  - inter_vec[1]) < min_val :
            ...
            normal_vec = np.array([0, -1, 0]).astype(np.float)
        ...
        if  abs(target_shape.min_p[2]  - inter_vec[2]) < min_val :
            ...
            normal_vec = np.array([0, 0, -1]).astype(np.float)
...
def main():
    ...
    for c in root.findall('shader'):
        ...
        if shade_type == 'Phong' :
            diff = np.array(c.findtext('diffuseColor').split()).astype(np.float)
            spec = np.array(c.findtext('specularColor').split()).astype(np.float)
            expo = np.array(c.findtext('exponent').split()).astype(np.float)
    ...
    for c in root.findall('surface'):
        ...
        if surface_type == 'Sphere' :
            ...
            pos = np.array(c.findtext('center').split()).astype(np.float)
            raidus =  np.array(c.findtext('radius').split()).astype(np.float) 
