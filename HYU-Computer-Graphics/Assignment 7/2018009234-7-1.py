...
def display():
    ...
    pos = time*(getTranslation(cow2wld2) - getTranslation(cow2wld1)) + getTranslation(cow2wld1)
   

    #drawCow(cow2wld1);														# Draw cow.
    #drawCow(cow2wld2);														# Draw cow.
    drawCow(matrix4(R3.as_matrix(), pos))
...
def main():
    ...
    width = 800;
    height = 600;
    window = glfw.create_window(width, height, '2018009234-7-1', None, None)
    if not window:
        glfw.terminate()
        sys.exit(-1)
