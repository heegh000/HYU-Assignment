	...
	if(delNode->leftChild == NULL && delNode->rightChild == NULL) {
		if(parentNode == NULL) 
			*p = NULL:
			...
			if(parentNode->leftNode == delNode) 
				parentNode->leftNode = NULL;
			else
				parentNode->rightNode = NULL;
		}
	}
	else if(delNode->leftChild == NULL || delNode->rightNode == NULL) {
		childNode = (delNode->leftNode != NULL) ? delNode->leftChild : delNode->rightChild;
